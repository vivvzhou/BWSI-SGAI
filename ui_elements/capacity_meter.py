...
class CapacityMeter(object):
    ...
    def update_fill(self, index):
        if index != 0:
            self.canvas.itemconfig(self.__units[index - 1], fill="green")
        else:
            for unit in self.__units:
                self.canvas.itemconfig(unit, fill="white")

def create_unit(canvas, x, y, size):
    return canvas.create_rectangle(x, y, x+size, y+size, fill='white')
