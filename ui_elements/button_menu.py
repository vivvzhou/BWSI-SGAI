...
class ButtonMenu(object):
    def __init__(self, root, items):
        ...
        self.canvas.place(x=100, y=110)
        self.buttons = create_buttons(self.canvas, items)
        create_menu(self.buttons)
        

    def disable_buttons(self, remaining_time, remaining_humanoids, at_capacity, remaining_hp):
        if remaining_humanoids == 0 or remaining_time <= 0:
            for i in range(0, len(self.buttons)):
                self.buttons[i].config(state="disabled")
        #  Not enough time left? Disable action
        if not at_capacity: #if not at capacity, check times
            self.buttons[0].config(state="normal") #enable all buttons if not at capcity, and then filter them out using times later
            ...
            if (remaining_time - ActionCost.CURE.value) < ActionCost.CURE.value:
        ...
        else: #at capacity, disabled everything
            self.buttons[0].config(state="disabled")
            self.buttons[1].config(state="disabled")
            self.buttons[2].config(state="disabled")
            self.buttons[3].config(state="disabled")

        if remaining_hp <= 150: #if hp too low then disable squish
            self.buttons[1].config(state="disabled")
